#!/usr/bin/env bash
# Cortex — Build 02, Lesson 04 — local + Docker cleanup
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${ROOT}"

echo "Local cleanup starting in: ${ROOT}"
...
for path in .venv __pycache__ .pytest_cache .mypy_cache .ruff_cache htmlcov .coverage model.joblib; do
...
if ! timeout 10 docker info >/dev/null 2>&1; then
  echo "Docker daemon is not running or did not respond — start Docker and re-run this script for Docker cleanup."
  exit 0
fi
